...
void Take_R1(char *R1, CondVar *C1, Lock *L1,char* AvailR, char* isSafe){
  ...
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();
//R1이 0이면 젓가락이 없어서 기다린다 or
//requestA가 AvailR보다 크고 & 젓가락 2개 갖고 있는 사람이 없으면 기다린다.
  while(Load(R1)==0 || ((MAX-AllocA)>Load(AvailR)&& Load(isSafe)==0)){
    if(Load(isSafe)==0)
      printf("**deadlock warning**Each one has one R except (A)\n");
...
void Take_R2(char *R2, CondVar *C2, Lock *L1,char* AvailR,char* isSafe){
  ...
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();
  //R2가 0이면 젓가락이 없어서 기다린다 or requestA가 AvailR보다 크면 기다린다.
  while(Load(R2)==0 || (MAX-AllocA)>Load(AvailR)){
...
void Put_R1(char *R1, CondVar *C1, Lock *L1,char* AvailR,char* isSafe){
  Acquire(L1);
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();
...
void Put_R2(char *R2, CondVar *C2, Lock *L1,char* AvailR){
  Acquire(L1);
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();
 ...
 void Phil_A(char *R1, char *R2, CondVar *C1, CondVar *C2, Lock *L1,char* AvailR,char* isSafe){
   pid_t pid;   //프로세서 id 받아옴
   pid = getpid();
   ...
   Put_R1(R1, C1, L1,AvailR,isSafe);   //젓가락 R1 내려놓는다.
   Put_R2(R2, C2, L1,AvailR);          //젓가락 R2 내려놓는다.
 }
...
//시작할때 pid와 시간 표시
  ...
  for(int i=0;i<100;i++){  //100번 밥먹음
    Phil_A(R1, R2, &C1, &C2, &L1,AvailR,isSafe);
