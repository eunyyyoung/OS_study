...
void Take_R3(char *R3, CondVar *C3, Lock *L1,char* AvailR,char* isSafe){
  ...
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();
  //R3가 0이면 젓가락이 없어서 기다린다 or
  //requestC가 AvailR보다 크고 & 젓가락 2개 갖고 있는 사람이 없으면 기다린다.
  while(Load(R3)==0 || ((MAX-AllocC)>Load(AvailR) && Load(isSafe)==0)){
    if(Load(isSafe)==0)
      printf("**deadlock warning**Each one has one R except (C)\n");
    printf("process %d (C) wait R3(take)\n", pid);
    Wait(C3, L1);
    printf("process %d (C) wakes up waiting for R3\n", pid);
  }
  Store(R3,0);
  printf("process %d (C) gets R3\n", pid);
  sub(AvailR, 1);
  AllocC++;
  Release(L1);
}

void Take_R1(char *R1, CondVar *C1, Lock *L1,char* AvailR,char* isSafe){
  Acquire(L1);
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();
//R1이 0이면 젓가락이 없어서 기다린다 or requestC가 AvailR보다 크면 기다린다
  while(Load(R1)==0 || (MAX-AllocC)>Load(AvailR)){
    printf("process %d (C) wait R1(take)\n", pid);
    Wait(C1, L1);
    printf("process %d (C) wakes up waiting for R1\n", pid);
  }
  Store(R1, 0);
  printf("process %d (C) gets R1\n", pid);
  sub(AvailR, 1);
  AllocC++;
...
void Put_R3(char *R3, CondVar *C3, Lock *L1,char* AvailR,char* isSafe){
  Acquire(L1);
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();

  Store(R3,1);
  printf("process %d (C) puts R3 down\n", pid);
  add(AvailR,1);
  AllocC--;
  sub(isSafe,1);  //젓가락 2개 갖는 사람 없다.
  Signal(C3);  //젓가락 R3 기다리는 사람 있으면 깨운다.
  printf("process %d (C) wait R3(put)\n", pid);
  Release(L1);
}

void Put_R1(char *R1, CondVar *C1, Lock *L1,char* AvailR){
  Acquire(L1);
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();

  Store(R1,1);
  printf("process %d (C) puts R1 down\n", pid);
  add(AvailR,1);
  AllocC--;
  Signal(C1); //젓가락 R1 기다리는 사람 있으면 깨운다.
  printf("process %d (C) wait R1(put)\n", pid);
  Release(L1);
}


void Phil_C(char *R3, char *R1, CondVar *C3, CondVar *C1, Lock *L1,char* AvailR,char* isSafe){
  pid_t pid;   //프로세서 id 받아옴
  pid = getpid();

  Take_R3(R3, C3, L1,AvailR,isSafe);  //젓가락 R3 집는다.
  printf("process %d (C) starts thinking\n", pid);
  sleep(1);  //thinking하는 시간
  printf("process %d (C) stops thinking\n", pid);
  Take_R1(R1, C1, L1,AvailR,isSafe);  //젓가락 R1 집는다.
  printf("process %d (C) starts eating\n", pid);
  sleep(2);  //eating하는 시간
  printf("process %d (C) stops eating\n", pid);
  Put_R3(R3, C3, L1,AvailR,isSafe);  //젓가락 R3 내려놓는다.
  Put_R1(R1, C1, L1,AvailR);         //젓가락 R1 내려놓는다.
}
...
void main() {
  ...
  d = localtime(&now);
//시작할때 pid와 시간 표시
  printf("\n-------- Phil_C Start - pid: %d,  time: %02d:%02d:%02d\n", pid, d->tm_hour, d->tm_min, d->tm_sec);
  ...
  for(int i=0;i<100;i++){  //100번 밥먹음
    Phil_C(R3, R1, &C3, &C1,&L1, AvailR, isSafe);
    printf("C finished eating (%d)\n",i+1);
  }
}
