...
//R2가 1이면 젓가락이 있고 0이면 젓가락이 없어서 기다려야 함
  while(Load(R2)==0){
    printf("process %d (C) wait R2(take)\n", pid);
    Wait(C2, L2);
    printf("process %d (C) wakes up waiting for R2\n", pid);
  }
  Store(R2,0);
  printf("process %d (C) gets R2\n", pid);
  Release(L2);
}
...
void Put_R2(char *R2, CondVar *C2, Lock *L2){
  ...
  Store(R2,1);
  printf("process %d (C) puts R2 down\n", pid);
  Signal(C2); //젓가락 R2 기다리는 사람 있으면 깨운다.
  printf("process %d (C) wait R2(put)\n", pid);
  Release(L2);
}
...
void Put_R3(char *R3, CondVar *C3, Lock *L3){
 ...
 void Phil_C(char *R3, char *R1, CondVar *C3, CondVar *C1, Lock *L3, Lock *L1){
   pid_t pid;   //프로세서 id 받아옴
   pid = getpid();

   Take_R3(R3, C3, L3);  //젓가락 R3 집는다.
