...
//R1이 1이면 젓가락이 있고 0이면 젓가락이 없어서 기다려야 함
  while(Load(R1)==0){
    printf("process %d (B) wait R1(take)\n", pid);
    Wait(C1, L1);
    printf("process %d (B) wakes up waiting for R1\n", pid);
  }
  Store(R1, 0);
  printf("process %d (B) gets R1\n", pid);
  Release(L1);
}
...
void Put_R1(char *R1, CondVar *C1, Lock *L1){
  ...
  Store(R1,1);
  printf("process %d (B) puts R1 down\n", pid);
  Signal(C1); //젓가락 R1 기다리는 사람 있으면 깨운다.
  printf("process %d (B) wait R1(put)\n", pid);
  Release(L1);
}
...
void Put_R3(char *R3, CondVar *C3, Lock *L3){
 ...
 void Phil_B(char *R2, char *R3, CondVar *C2, CondVar *C3, Lock *L2, Lock *L3){
   pid_t pid;   //프로세서 id 받아옴
   pid = getpid();
